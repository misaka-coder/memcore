...
MemCore 是**开源软件**，采用**双许可证（dual licensing）**：[AGPL-3.0-only](LICENSE) **或** 商业许可证，使用者可任选其一。SPDX 标识：`AGPL-3.0-only OR LicenseRef-Commercial`。
...
**开源侧（AGPL-3.0）**：可以在遵守条款的前提下自由使用、修改、分发，包括商业用途。最需要注意的义务是**第 13 条**——如果你修改了 MemCore 并把它作为网络服务提供给别人使用，需要向他们提供你所修改版本的对应源码。内部自用不触发分发义务。

**商业侧**：如果 AGPL 的义务不适合你的产品形态（典型是闭源分发或对外 SaaS），可以另行取得商业许可证，条款与费用单独约定。范围、常见误区与联系方式见 [`COMMERCIAL.md`](COMMERCIAL.md)。

几点说明：

- 本库不含受私有许可约束的人格/权重内容——人格由接入方运行时注入，因此可作为**纯机制**独立授权。
- Akane 的对外开源项目采用更宽松的许可证，与 MemCore 彼此独立、互不影响；MemCore 的 copyleft 义务不会因"上层项目更宽松"而自动扩大或缩小，但也**不会因为分层设计而自动消失**——直接复制其源码或链接进同一二进制仍会触发义务。
