...
`max_segment_chars` 是软目标：只在成对符号之外的标点或空白处分段，找不到安全边界时允许整段超过目标，不从词语中间强切。括号、书名号、单双引号和反引号代码标记按闭合栈处理，内部换行也保持同段；英文缩写中的撇号（如 `don't`）不会开启引号。源文本未闭合时在结束阶段原样交付，不补造闭合符号。宿主可以另外细分音频请求，但应保留完整显示片段与音频块的对应关系。
